...
export interface EVMCollectionStore {
  ...
  duplicateContract: (id: string, parentId: string) => void;
...
export const useEVMCollectionStore = create<EVMCollectionStore>()(
  persist(
    (set) => ({
      ...
      duplicateContract: (id: string, parentId: string) =>
        set((state) => {
          const item = findItemInCollections(state.collections, id);
          const parent = findItemInCollections(state.collections, parentId);

          if (item && item.type === EVMItemType.SmartContract && parent && parent.type !== EVMItemType.SmartContract) {
            const cpItem = { ...item, id: uuidv4() };
            parent.items.push(cpItem);
          }
